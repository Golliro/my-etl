...
column_template = ['department_name', 'sensor_serial',
                   'create_at', 'product_name', 'product_expire']
number_of_department = 100
sensor_in_department = [random.choices(
    range(5, 30))[0] for i in range(number_of_department)]
number_of_sensor = sum(sensor_in_department)
number_of_product = 1000
department_list = gen_text(
    number_of_item=number_of_department, long_of_text=32, text_list=list_char)
sensor_list = gen_text(number_of_item=number_of_sensor,
                       long_of_text=64, text_list=list_char)
product_list = gen_text(number_of_item=number_of_product,
                        long_of_text=16, text_list=list_char)
...
for i in range(number_of_department):
    ...
    data_department = pd.DataFrame({
        'department_name': [d_name] * n_sensor, 'sensor_serial': random.choices(
            [s for s in sensor_list if s not in check_list], k=n_sensor
        )
    })
    data_template = pd.concat(
        [data_template, data_department], ignore_index=True)
del data_department
...
for i in tqdm(range(len(list_date))):
    ...
    data_date['product_name'] = random.choices(
        product_list, k=number_of_sensor)
    data_date['product_expire'] = list(map(
        lambda x: x + dt.timedelta(days=90 - random.choices([1, 2, 3])[0]), data_date['create_at']))
